...
$ helm repo update
$ helm search repo appscode/prepare-cluster --version=v0.2.0
$ helm upgrade -i prepare-cluster appscode/prepare-cluster -n kube-system --create-namespace --version=v0.2.0
```
...
| gke.copyStorageClasses     |                                                                                                                                                                                                                                              | <code>{}</code>                                                                                                                                                                                                                        |


Specify each parameter using the `--set key=value[,key=value]` argument to `helm upgrade -i`. For example:

```bash
$ helm upgrade -i prepare-cluster appscode/prepare-cluster -n kube-system --create-namespace --version=v0.2.0 --set preparer.repository=tianon/toybox
```
...
```bash
$ helm upgrade -i prepare-cluster appscode/prepare-cluster -n kube-system --create-namespace --version=v0.2.0 --values values.yaml
```
