...
[Kubernetes Storage Metrics API Server by AppsCode](https://github.com/kubeops/storage-metrics-apiserver) - Kubernetes Storage Metrics API server by AppsCode
...
$ helm repo update
$ helm search repo appscode/storage-metrics-apiserver --version=v2026.7.8
$ helm upgrade -i storage-metrics-apiserver appscode/storage-metrics-apiserver -n kubeops --create-namespace --version=v2026.7.8
```
...
To install/upgrade the chart with the release name `storage-metrics-apiserver`:
...
To uninstall the `storage-metrics-apiserver`:

```bash
$ helm uninstall storage-metrics-apiserver -n kubeops
```
...
The following table lists the configurable parameters of the `storage-metrics-apiserver` chart and their default values.
...
| registryFQDN                             | Docker registry FQDN used to pull the storage-metrics-apiserver image. Set this to use a docker registry hosted at ${registryFQDN}/${registry}/${image} | <code>ghcr.io</code>                   |
| image.registry                           | Docker registry used to pull the image                                                                                                                  | <code>appscode</code>                  |
| image.repository                         | Name of the container image                                                                                                                             | <code>storage-metrics-apiserver</code> |
...
```bash
$ helm upgrade -i storage-metrics-apiserver appscode/storage-metrics-apiserver -n kubeops --create-namespace --version=v2026.7.8 --set registryFQDN=ghcr.io
```
...
```bash
$ helm upgrade -i storage-metrics-apiserver appscode/storage-metrics-apiserver -n kubeops --create-namespace --version=v2026.7.8 --values values.yaml
```
