...
$ helm repo update
$ helm search repo appscode/fargocd-manager --version=v2026.9.18
$ helm upgrade -i fargocd-manager appscode/fargocd-manager -n open-cluster-management-fargocd --create-namespace --version=v2026.9.18
```
...
| registryFQDN                             | Docker registry fqdn used to pull fargocd images on every spoke.                                                                                                    | <code>ghcr.io</code>                  |
...
| argocd.destServer                        | Left empty; fargocd's controller falls back to the in-cluster server when both this and destName are empty. Must stay empty if destName is set.                     | <code>""</code>                       |
...
| argocd.kubeconfig                        | Raw kubeconfig for the Argo CD principal (managed mode); the manager mounts it and propagates it to every spoke. Mutually exclusive with kubeconfigSecret.          | <code>""</code>                       |
| argocd.kubeconfigSecret                  | Name of an existing Secret (data key `kubeconfig`) holding the Argo CD principal kubeconfig (managed mode). Mutually exclusive with argocd.kubeconfig.              | <code>""</code>                       |
| argocd.kubeconfigSpokeSecret             | Name of a pre-created Secret on each spoke holding the principal kubeconfig, instead of the manager pushing the inline kubeconfig.                                  | <code>""</code>                       |
| kubeconfigSecretName                     | Name of a Secret holding an OCM hub kubeconfig; when set, the manager uses --kubeconfig instead of the in-cluster ServiceAccount.                                   | <code>""</code>                       |
...
```bash
$ helm upgrade -i fargocd-manager appscode/fargocd-manager -n open-cluster-management-fargocd --create-namespace --version=v2026.9.18 --set registryFQDN=ghcr.io
```
...
```bash
$ helm upgrade -i fargocd-manager appscode/fargocd-manager -n open-cluster-management-fargocd --create-namespace --version=v2026.9.18 --values values.yaml
```
